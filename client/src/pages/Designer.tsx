    ...
    reader.onload = (event) => {
      ...
      try {
        if (fileName.endsWith('.json')) {
          ...
          const { nodes, edges, projectName: parsedName, computationalParams: parsedParams, pcharData, tcharData, vSchedules, outputRequests: parsedRequests, nodeSelectionSet: parsedNodeSel } = parseInpFile(content);
          if (nodes.length > 0) {
            const loadedProjectName = parsedName || file.name.replace(/\.inp$/i, '');
            loadNetwork(nodes, edges, parsedParams, parsedRequests, loadedProjectName, undefined, pcharData, undefined, parsedNodeSel, tcharData, vSchedules);
            setProjectState("active");
            setServerProjectId(null);
    ...
    if ('showOpenFilePicker' in window) {
      try {
        ...
        if (fileName.endsWith('.json')) {
          ...
          const { nodes: n, edges: e, projectName: pn, computationalParams: cp, pcharData: pd, tcharData: td, vSchedules: vs, outputRequests: reqs, nodeSelectionSet: nodeSel } = parseInpFile(content);
          if (n.length > 0) {
            const name = pn || file.name.replace(/\.inp$/i, '');
            loadNetwork(n, e, cp, reqs, name, handle, pd, undefined, nodeSel, td, vs);
            setProjectState("active");
            setServerProjectId(null);
            setShowProjectsList(false);
